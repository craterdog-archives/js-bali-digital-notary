...
const DigitalNotary = function(securityModule, account, directory, debug) {
     ...
     * process or service.
     * 
     * @param {Tag} salt A random tag that was generated by the process or service requesting
     * authentication.
    ...
    /**
     * This method digitally signs the specified document using the notary key maintained by
     * the security module. The document must be parameterized with the following parameters:
     * <pre>
     ...
     * @returns {Catalog} A newly notarized document containing the document.
     */
    this.notarizeDocument = async function(document) {
        try {
            ...
            if (debug > 1) {
                ...
                validateStructure('$notarizeDocument', 'component', document);
            }
        ...
        switch (parameterType) {
            ...
            case 'certificate':
                ...
                if (parameterValue.isComponent && parameterValue.isType('/bali/collections/Catalog') && parameterValue.getSize() === 2) {
                    ...
                    validateStructure(functionName, parameterName + '.algorithms', parameterValue.getValue('$algorithms'), 'component');
                    const parameters = parameterValue.getParameters();
